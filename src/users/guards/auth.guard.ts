...
async canActivate(context: ExecutionContext) {
  ...
  if (token && type === "Bearer") { //check
    try {
      const payload: jWTPayloadType = await this.jwtService.verifyAsync(token, { //Validates the token using your JWT secret ,Extracts the payload { id: 1, email: "test@test.com" })
        secret: this.configservice.get<string>("JWT_SECRET"),
      });

      // attach user payload to request
      request["user"] = payload; //id here
